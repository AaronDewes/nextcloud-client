...
export default class File implements FileSystemElement {
  ...
  public async getFolder(): Promise<Folder> {
    this.assertExistence();
    const folder: Folder | null = await this.client.getFolder(
      path.dirname(this.memento.name),
    );
    ...
    throw new ClientError(
      "Error, the folder of the file does not exist anymore",
      "ERR_FILE_FOLDER_DOES_NOT_EXIST",
    );
  }
   ...
   * @throws Error
   */
  public getContent(): Promise<Buffer> {
    this.assertExistence();
   ...
   */
  public addTag(tagName: string): Promise<void> {
    this.assertExistence();
