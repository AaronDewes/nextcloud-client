...
export default class Folder implements FileSystemElement {
  ...
  public async hasSubFolder(subFolderBaseName: string): Promise<boolean> {
    this.assertExistence();
    const subFolder: Folder | null = await this.client.getFolder(
      this.name + "/" + subFolderBaseName,
    );
    if (subFolder) {
      return true;
    }
   ...
   * @throws Error
   */
  public getFile(fileBaseName: string): Promise<File | null> {
    this.assertExistence();
  ...
  public createFile(
    ...
    return this.client.createFile(
      this.name + "/" + fileBaseName.replace(/^\/+/g, ""),
      data,
    );
  }
  ...
  public async move(targetFolderName: string): Promise<Folder> {
    this.assertExistence();
    const folder: Folder = await this.client.moveFolder(
      this.name,
      targetFolderName,
    );
  ...
  public async containsFile(fileBaseName: string): Promise<boolean> {
    this.assertExistence();
    const file: File | null = await this.getFile(fileBaseName);
    if (file) {
      return true;
    }
    return false;
