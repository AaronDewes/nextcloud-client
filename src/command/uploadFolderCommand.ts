...
import Command, { CommandStatus } from "./command.ts";
...
export default class UploadFolderCommand extends Command {
  ...
  constructor(client: Client, options: UploadFolderCommandOptions) {
    ...
    if (options.getTargetFileNameBeforeUpload) {
      this.getTargetFileNameBeforeUpload =
        options.getTargetFileNameBeforeUpload;
    } else {
      this.getTargetFileNameBeforeUpload = (
        fileNames: SourceTargetFileNames,
      ): string => {
        return fileNames.targetFileName;
      };
    }
  ...
  protected async onExecute(): Promise<void> {
    ...
    for (const fileNameFormat of fileNames) {
      const targetFileName = this.getTargetFileNameBeforeUpload({
        sourceFileName: fileNameFormat.absolute,
        targetFileName: fileNameFormat.relative,
      });
    ...
    const options: UploadFilesCommandOptions = {
      files,
      processFileAfterUpload: this.processFileAfterUpload,
    };
