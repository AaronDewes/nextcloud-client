...
const debug = debugInit.debug("DownloadFilesCommand");
...
export default class DownloadFolderCommand extends Command {
  ...
  constructor(client: Client, options: DownloadFolderCommandOptions) {
    ...
    this.getTargetFileNameBeforeDownload =
      options.getTargetFileNameBeforeDownload;
  ...
  protected async onExecute(): Promise<void> {
    ...
    try {
      ...
      for (const file of files) {
        const targetFileName = this.getTargetFileNameBeforeDownload({
          sourceFileName: file.name,
          targetFileName: "." + file.name,
        });
        const content: Buffer = await file.getContent();
        const path: string = targetFileName.substring(
          0,
        ...
        this.percentCompleted = (this.bytesDownloaded / bytesToDownload * 80) +
          20;
      }
