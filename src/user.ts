...
import { UserGroup } from "./client.ts";
import FileSizeFormatter from "./fileSizeFormatter.ts";
...
export default class User {
  ...
  async setDisplayName(value: string): Promise<void> {
    await this.client.updateUserProperty(
      this.id,
  ...
  async isSuperAdmin(): Promise<boolean> {
    return (await this.getUserData()).memberGroups.indexOf("admin") === -1
      ? false
      : true;
  }
