...
(async () => {
  try {
    ...
    // set some properties (... password, phone, website, twitter, address, email, locale)
    await user.setDisplayName("My Display Name");
    await user.setQuota("5 GB");
    await user.setLanguage("en");
    // get properties (... quota, user friendly quota, phone, website, twitter, address, locale)
    ...
    // use specific exception *error classes for error handling documented in @throws
  }
})();
