  ...
  public constructor(instanceName: string) {
    if (!process.env.VCAP_SERVICES) {
      throw new ClientError(
        "NCEnvironmentVcapServices: environment VCAP_SERVICES not found",
        "ERR_VCAP_SERVICES_NOT_FOUND",
      );
    }

    const vcapServices = require("vcap_services");
    const cred = vcapServices.getCredentials(
      "user-provided",
      undefined,
      instanceName,
    );
    ...
    if (
      ...
      log.error(
        "NCEnvironmentVcapServices: error credentials not found or not fully specified ",
        cred,
      );
      throw new ClientError(
        `NCEnvironmentVcapServices: nextcloud credentials not found in environment VCAP_SERVICES. Service section: "user-provided", service instance name: "${instanceName}" `,
        "ERR_VCAP_SERVICES_NOT_FOUND",
      );
    }
    ...
    if (!cred.url) {
      throw new ClientError(
        "NCEnvironmentVcapServices: VCAP_SERVICES url not defined in user provided services for nextcloud",
    ...
    if (!cred.password) {
      throw new ClientError(
        "NCEnvironmentVcapServices: VCAP_SERVICES password not defined in user provided services for nextcloud",
    ...
    if (!cred.username) {
      throw new ClientError(
        "NCEnvironmentVcapServices: VCAP_SERVICES username not defined in user provided services for nextcloud",
