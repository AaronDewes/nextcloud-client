import type { IRequestContext } from "./httpClient.ts";
...
export default class FakeServer {
  ...
  public async getFakeHttpResponse(
    _url: string,
    ...
    const rrEntry: RequestResponseLogEntry | undefined = this.fakeResponses
      .shift();
    if (!rrEntry) {
      throw new Error(
        `error providing fake http response. No fake response available`,
      );
    }
    ...
    if (rrEntry.response.contentLocation) {
      response.headers.append(
        "Content-Location",
        rrEntry.response.contentLocation,
      );
    }

    if (expectedHttpStatusCode.indexOf(response.status) === -1) {
      log.debug(
        "getHttpResponse unexpected status response ",
